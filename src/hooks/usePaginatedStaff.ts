...
const applyNameFilter = (items: BulkStaff[], name: string): BulkStaff[] => {
  ...
  return items.filter((s) => (s.Forename?.toLowerCase() ?? "").includes(q));
};
...
const buildQueryConstraints = (
  ...
  console.log(
    "[usePaginatedStaff] buildQueryConstraints:",
    describeConstraints(c),
  );
  return c;
};
...
export const usePaginatedStaff = ({
  ...
  const fetchPage = useCallback(
    async (pageNum: number) => {
      ...
      if (pageNum === 1) {
        console.log("[usePaginatedStaff] ===== fetchPage(1) called =====", {
          agencyId,
          assignedToId,
        ...
        try {
          ...
          console.log(
            "[usePaginatedStaff] count result:",
        ...
        try {
          ...
          console.log(
            "[usePaginatedStaff] page 1 returned docs:",
            snap.docs.length,
            snap.docs.map((d) => ({
              id: d.id,
              forename: data(d).Forename,
              surname: data(d).Surname,
              email: data(d).email,
              agencyId: data(d).agencyId,
            })),
          );
          if (cancelledRef.current) return;
          ...
          const lastCursor = docs.length > 0 ? docs[docs.length - 1].id : null;
      ...
      try {
        ...
        const data = (d: { data: () => unknown }) => d.data() as DocumentData;
        console.log(
          `[usePaginatedStaff] page ${pageNum} returned docs:`,
          snap.docs.length,
          snap.docs.map((d) => ({
            id: d.id,
            forename: data(d).Forename,
